...
const createCourse = (req, res) => {
    ...
    sequelize.transaction(async (t) => {
      ...
      const [result, meta] = await sequelize.query(insertSql, { replacements: params, transaction: t, type: QueryTypes.INSERT });
      let courseId = (meta && meta.insertId) || (result && result.insertId);
      if (!courseId) {
        const idRows = await sequelize.query('SELECT LAST_INSERT_ID() AS id', { transaction: t, type: QueryTypes.SELECT });
      ...
      if (tagIds.length > 0) {
        ...
        await sequelize.query(tagQuery, { replacements: flat, transaction: t, type: QueryTypes.INSERT });
      ...
      return res.status(500).json({ error: 'Internal server error. ' + (err && err.message ? err.message : err) });
    });
...
`;
    ...
    sequelize.query(quer, { replacements: params, type: QueryTypes.SELECT })
      .then((results) => res.status(200).json(results))
...
const getCourseById = (req, res) => {
    ...
    const quer = req.user.role === 'admin'
...
const updateCourse = (req, res) => {
    ...
    const candidateKeys = ['name','description','price','teacher_id','date','time'];
    ...
    sequelize.transaction(async (t) => {
      ...
      if (keys.length > 0) {
        ...
        if (req.user.role === 'admin') {
          ...
          const [resMeta] = await sequelize.query(sql, { replacements: [...values, req.body.id], transaction: t, type: QueryTypes.UPDATE });
          const meta = Array.isArray(resMeta) ? resMeta[1] : resMeta;
          updatedRows = (meta && (meta.affectedRows || meta.rowCount)) || 0;
          if (updatedRows === 0) {
            const rows = await sequelize.query(
              `SELECT id FROM courses WHERE id = ?`,
              { replacements: [req.body.id], transaction: t, type: QueryTypes.SELECT }
            );
            existsWithinScope = Array.isArray(rows) && rows.length > 0;
          }
        } else {
          const sql = `UPDATE courses SET ${setClause} WHERE id = ? AND teacher_id = ?`;
          const [resMeta] = await sequelize.query(sql, { replacements: [...values, req.body.id, req.user.id], transaction: t, type: QueryTypes.UPDATE });
          const meta = Array.isArray(resMeta) ? resMeta[1] : resMeta;
          updatedRows = (meta && (meta.affectedRows || meta.rowCount)) || 0;
          if (updatedRows === 0) {
            const rows = await sequelize.query(
              `SELECT id FROM courses WHERE id = ? AND teacher_id = ?`,
              { replacements: [req.body.id, req.user.id], transaction: t, type: QueryTypes.SELECT }
            );
            existsWithinScope = Array.isArray(rows) && rows.length > 0;
          }
        }
      ...
      if (Array.isArray(tags)) {
        ...
        await sequelize.query(
          `DELETE FROM courses_tags WHERE course_id = ?`,
          { replacements: [req.body.id], transaction: t, type: QueryTypes.DELETE }
        );
      ...
      return { updatedRows, tagsChanged, tagsCount, hadFieldUpdates: keys.length > 0, existsWithinScope };
    }).then((outcome) => {
      ...
      if (outcome.hadFieldUpdates && outcome.tagsChanged) {
        ...
        if (outcome.updatedRows === 0 && outcome.existsWithinScope) {
          return res.status(200).json({ message: 'No changes detected. Tags updated successfully.' });
        }
      ...
      if (!outcome.hadFieldUpdates && outcome.tagsChanged) {
        ...
        if (outcome.tagsCount && outcome.tagsCount > 0) {
          return res.status(200).json({ message: 'Course tags updated successfully.' });
        }
...
const createTag = (req,res)=> {
    ...
    sequelize.query(query, { replacements: [tag.name] })
