...
const login = (req, res) => {
  ...
  sequelize
    .query(query, { replacements: [email], type: QueryTypes.SELECT })
    .then((results) => {
      ...
      bcrypt.compare(password, user.password, (err, matched) => {
        ...
        return res.status(200).json({
          ...
          token, // normal token
          adminToken, // admin token if role = admin
    ...
    .then((results) => {
      ...
      bcrypt.hash(userData.password, 10, (err, hash) => {
        ...
        const values = [
          ...
          userData.role,
          userData.name,
          userData.national_number,
          userData.Grade || null,
        ];
        sequelize
          .query(insertSql, { replacements: values, type: QueryTypes.INSERT })
          .then(() =>
...
const user = (req, res) => {
  ...
  sequelize
    .query(quer, { replacements: [req.user.id], type: QueryTypes.SELECT })
...
const upgradeRole = (req, res) => {
  ...
  sequelize
    .query(quer, {
      replacements: [req.body.role, req.body.id],
